...
chat <- read_lines(here("data", "meeting_saved_chat.txt"))
chat
length(chat)
...
chat_df <- tibble(
  ...
  mutate(first = str_extract(from, "^\\w+?(\\s|$)"),
         first = str_trim(first),
         pronouns = str_extract(from, "\\(.+?\\)"), 
         pronouns = str_remove_all(pronouns, "(\\(|\\))")) %>% 
  select(time, first, pronouns, message, from)
