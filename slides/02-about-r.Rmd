...
# .top[Using RMarkdown to .kjh-orange[produce] and .kjh-green[reproduce] your work]
...
# .middle[<br />GETTING <br />O.kjh-orange[R]IENTED]
...
- .SMALL[Code you can type and run:] 

```{r}
## Inside code chunks, lines beginning with a # character are comments
## Comments are ignored by R

my_numbers <- c(1, 1, 2, 4, 1, 3, 1) # Anything after a # character is ignored as well
...
- .smaller[By convention, code output in documents is prefixed by `##`]
...
- .smaller[Also by convention, outputting vectors, etc, gets a counter keeping track of the number of elements. For example,] 
...
# .top.large[SOME THINGS<br />TO KNOW<br />ABOUT .kjh-orange[R]] 
...
- Logical equality and inequality (yielding a .kjh-green[`TRUE`] or .kjh-green[`FALSE`] result) is done with `==` and `!=`. Other logical operators include `<`, `>`, `<=`, `>=`, and `!` for negation.
...
- .small[Here's a gotcha. You might think you could write `3 < 5 & 7` and have it be interpreted as "Three is less than five and also less than seven":]

```{r}
3 < 5 & 7
```

- .small[It seems to work!]

---

# .kjh-red[Watch out!]

- .small[But now try `3 < 5 & 1`, where your intention is "Three is less than five and also less than one"]
...
- .smaller[What's happening is that `3 < 5` is being evaluated first, and resolving to .kjh-green[TRUE], leaving us with the expression .kjh-green[`TRUE`] `& 1`, which R is effectively interpreting as .kjh-green[`TRUE`] `& as.logical(1)`. In Boolean algebra, `0` resolves to  .kjh-green[`FALSE`] and any other number to .kjh-green[`TRUE`]. So you have to make your comparisons explicit:]
...
- Welcome to floating point maths!
...
- .small[When you do binary math on repeating numbers and convert back to decimals you get tiny leftovers, and this can mess up _logical_ comparisons of equality. The `all.equal()` function exists for this purpose.]
...
# .top.small[More later on why this might bite you, and how to deal with it]
...
```{r}
my_numbers ## We just created this
...
- You can assign the output of a function to a name, which turns it into an object
...
- You need only _install_ a package once (or occasionally update it). But you must _load_ the package in each R session before you can access its contents
...
# Getting a single function with `::`

- .small[You can, however, "reach in" to an unloaded package and grab a function directly, using `::` between the name of the package and the function you want, like this: `<package>::<function>`]
