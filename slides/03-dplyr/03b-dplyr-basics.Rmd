...
# Recapping Yesterday and Looking Ahead
...
## Core .kjh-orange[`dplyr`] verbs

- Subset your table: .kjh-green[`filter()`] rows, .kjh-green[`select()`] columns
- Logically .kjh-green[`group_by()`] one or more columns
- Add columns with .kjh-green[`mutate()`]
- Summarize (by group, or the whole table) with .kjh-green[`summarize()`]
...
## Expand your .kjh-orange[`dplyr`] actions

.pull-left.w90[
- Count up rows with .kjh-green[`n()`], .kjh-green[`tally()`] or .kjh-green[`count()`]

- Calculate quantities with .kjh-green[`sum()`], .kjh-green[`mean()`], .kjh-green[`min()`], etc

- Subset rows with logical expressions or .kjh-green[`slice`] functions

- Conditionally select columns by name directly, with .kjh-green[`%in%`] or .kjh-green[`%nin%`], or with tidy selectors like .kjh-green[`starts_with()`], .kjh-green[`ends_with()`], .kjh-green[`contains()`]

- Conditionally select columns by _type_ with .kjh-green[`where()`] and some criterion, e.g. .kjh-green[`where(is.numeric)`]

- Conditionally select and then _act_ on columns with .kjh-green[`across(where(`].kjh-orange[`<condition>`].kjh-green[`),`] .kjh-orange[`<action>`].kjh-green[`)`]
]
---
...
- Tidy up columns with .kjh-green[`relocate()`] and .kjh-green[`rename()`]

- Tidy up rows with .kjh-green[`arrange()`]
...
```{r 03b-dplyr-basics-by-4}
gss_sm |> 
  summarize(total = n(), .by = c(bigregion, religion))
```

Notice that by default the result is an _ungrouped_ tibble, whereas with `group_by()` ... `summarize()` the result would still be grouped by `bigregion` at the end. To prevent unexpected results, you can't use `.by` on tibble that's already grouped.


---

# The data is implicitly the first argument

This code:

```{r 03b-dplyr-basics-by-5}
gss_sm |> 
  summarize(total = n(), .by = c(bigregion, religion))
```
...
# .middle.squish4.huge[Two dplyr .kjh-red[gotchas]]
...
# 1. Comparisons filtering on proportions
...
# 2. .kjh-orange[Zero Counts] in dplyr
