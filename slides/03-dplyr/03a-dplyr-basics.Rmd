...
- .kjh-orange[**Subset**] either the rows or columns of or table—i.e. remove them before doing anything.
...
When telling .kjh-green[**`where()`**] to use .kjh-green[**`is.integer()`**] to test each column, we don't put parentheses at the end of its name. If we did, R would try to evaluate .kjh-green[**`is.integer()`**] right then, and fail:
...
.SMALL.squish3[Instead, use .kjh-green[`across()`] to apply a function to more than one column.]
