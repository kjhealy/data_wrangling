...
```{r 06-getting-data-in-1, include=FALSE}
library(flipbookr)
library(here)
library(tidyverse)
library(kjhslides)
...
.SMALL[
```{r 06-getting-data-in-2, message = TRUE}
...
# .huge.middle.squish4[.kjh-orange[We've put a lot of pieces in place at this point]]
...
# An example: .kjh-green[**`read_table()`**]
...
```{r 06-getting-data-in-10, message=TRUE}
engmort <- read_table(here("data", "mortality.txt"), 
                      skip = 2, na = ".")
...
```{r 06-getting-data-in-11, include = FALSE}
...
`r chunk_reveal("06-getting-data-in-11", widths = c(60,40), title = "# Normalizing names and recoding")`
...
```{r 06-getting-data-in-12, warning = TRUE}
...
# Take a look with .kjh-green[`tail()`]
...
# Take a look with .kjh-green[`slice_sample()`]
...
# Take a look with .kjh-green[`slice()`]
...
# Take a look  with .kjh-green[`select()`] and .kjh-green[`filter()`]
...
# Take a look with .kjh-green[`is.na()`]
...
# Take a look with .kjh-green[`distinct()`]
...
# Take a look with .kjh-green[`read_lines()`]
...
```{r 06-getting-data-in-26, message = TRUE}
nchs <- with_edition(1, read_csv(here("data", "SAS_on_2021-04-13.csv"))) 
```
...
When .kjh-green[**`read_csv()`**] warns you of a parsing failure, **don't ignore it**.
...
```{r 06-getting-data-in-30, echo = FALSE}
nchs_fmt <- nchs %>% 
  select(-year, -month) %>% 
...
```{r 06-getting-data-in-31, include=FALSE}
...
`r chunk_reveal("06-getting-data-in-31", widths = c(50,50), title = "# If we wanted to ...")`
...
```{r 06-getting-data-in-40, include = FALSE}
...
`r chunk_reveal("06-getting-data-in-40", widths = c(40,60), title = "# Cleaning a table")`
...
# .huge.middle.squish4[.kjh-orange[What about Stata?]]
...
```{r 06-getting-data-in-61, include = FALSE}
gss_sub %>%
    mutate(agequint = cut(x = age, 
                          breaks = unique(age_quintiles), 
                          include.lowest = TRUE)) %>% 
...
`r chunk_reveal("06-getting-data-in-61", widths = c(45,55), title = "# The GSS panel: more recoding")`
...
```{r 06-getting-data-in-67}
gss_sub %>% 
  count(degree)
```

```{r 06-getting-data-in-68}
