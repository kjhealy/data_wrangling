...
- .center.SMALL[_By far_ the best way to get a feel for R works, and to get comfortable with the cycle of<br/> try-fail-cry-fix-wail-retry that is a permanent part of writing any code.]
...
.center[![:scale 100% "Tidyverse loading"](img/tidyverse-first.png)]

---

# Tidyverse components

.center[![:scale 100% "Tidyverse loading 2"](img/tidyverse-first-captioned.png)]
...
# Use the pipe operator: .kjh-yellow[%>%]
...
# Coming soon to an R near you: .kjh-yellow[|>]

- The pipe operator **`%>%`** is not part of Base R. It was introduced in a package called `magrittr`. 

--

- It's been so successful, a version of it will soon be incorporated into Base R.

--

- It will be denoted by **`|>`** and should be a direct replacement for **`%>%`** in almost every case

--

- The magrittr pipe will continue to work, though.
...
- Related concept of _namespaces_ and _environments_.
...
# 5. .kjh-yellow[Objects come in types and classes]
...
# Some things can't be directly combined
...
my_new_vector # vectors are homogeneous
...
- .SMALL.squish2[Tidyverse tools are generally _type safe_, meaning their functions return the same type of thing every time, and often require the same type of input that they return as output. So it's good to know about the various data types.]
