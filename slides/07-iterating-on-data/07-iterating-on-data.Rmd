...
```{r 07-iterating-on-census-5, message=FALSE, results='hide'}
df <- 2005:2019 %>% 
...
```{r 07-iterating-on-census-6, message=FALSE, results='hide'}
df <- 2005:2019 %>% 
  set_names() %>% 
  map_dfr(~ get_acs(geography = "county", 
                    variables = "B19013_001",
...
# Example: Iterating on the .kjh-yellow[US Census]
```{r 07-iterating-on-census-7, message=FALSE, results='hide'}
p_out <- 2005:2019 %>% 
  set_names() %>% 
...
```{r 07-iterating-on-census-7a, fig.width=12, fig.height=5}
print(p_out)
```
