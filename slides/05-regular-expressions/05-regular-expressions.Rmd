...
# .middle.squish4.large[.kjh-blue[Regular]<br />.kjh-blue[Expressions]]
...
.SMALL.squish2[A regular expression is a way of searching for a piece of text, or _pattern_, inside some larger body of text, called a _string_.] 

--

.SMALL.squish2[The simplest sort of search is like the "Find" functionality in a Word Processor, where the pattern is a literal letter, number, punctuation mark, word or series of words and the text is a document that gets searched one line at a time. The next step up is "Find and Replace".]

--

.SMALL.squish2[Every pattern-searching function in `stringr` has the same basic form:]
...
.SMALL.squish2[Functions that _replace_ as well as _detect_ strings all have this form:]
...
.smaller.squish3[(If you think about it, .kjh-orange[`<STRING>`], .kjh-orange[`<PATTERN>`] and .kjh-orange[`<REPLACEMENT>`] above are all kinds of pattern: they are meant to "stand for" all kinds of text, not be taken literally.)]
...
.SMALL[Regular expressions get their real power from _wildcards_, i.e. tokens that match more than just literal strings, but also more general and more complex patterns.] 

--

.SMALL[The most general pattern-matching token is, "Match everything!". This is represented by the period, or **`.`**]

--

.SMALL[But ... if “**`.`**” matches any character, how do you specifically match the character “**`.`**”?]
...
# But ... then how do you match a .kjh-yellow[literal] **`\`**?
...
By default these are _greedy_ matches. You can make them “lazy”, matching the shortest string possible by putting a **`?`** after them. 
